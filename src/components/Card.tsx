...
export const Card = ({  products, title }: Props) => {
    ...
    const handleColor = (color: string) => {
        setSelecProduct(products.find(item => item.color === color) || products[0])
             ...
             <div className="flex justify-between">
                ...
                <div className="flex items-center mb-2">
                    <div className="flex space-x-2">
                        {colors.map((color, index) => (
                            <button 
                                ...
                                className={`w-5 cursor-pointer hover:scale-110 h-5 text-sm font-medium transition-all duration-300 ${ getColor(color)
                                }`}
                            >
                  ...
                  <div className="flex items-center justify-between w-full">
                    <div className="flex space-x-2 ">
                        {selecProduct.sizes.map((size, i) => (
                            <button
                                key={i}
                                className="italic bg-gray-100 px-3 py-1 rounded text-sm font-medium transition-colors duration-300"
                            >
                                {size.toUpperCase()}
                            </button>
                        ))}
                    </div>
                        <button className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white text-sm py-1 px-2 ">
                    Agregar
                </button>
                </div>
